...
def change_bitrate(file):
    ...
    def get_bitrate(file):
        ...
        video_bitrate = int(video_stream['bit_rate']) if video_stream and 'bit_rate' in video_stream else None
        audio_bitrate = int(audio_stream['bit_rate']) if audio_stream and 'bit_rate' in audio_stream else None
    ...
    (
        ffmpeg
        .input(file)
        .output(output_file, video_bitrate=new_video_bitrate_str, audio_bitrate=new_audio_bitrate_str, acodec='aac')
        .run()
    )
